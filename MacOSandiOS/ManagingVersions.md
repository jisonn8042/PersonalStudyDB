...
前、なんで.zprofileの設定が必要なのかに関する説明はここでzshrcにも関係あるので一緒に説明します。

rbenvは色んな違うバージョンのRubyを正しいバージョンで実行するよう管理してくれるツールで、

ターミナルにrubyが必要なコマンドが入力されると、ディレクトリに.ruby-versionファイルが存在している場合

そのファイルに記載されているRubyバージョンで実行してくれます。

rbenvを使うには

.zshrc、.zprofileはターミナルを起動するとき実行されます。

MacOSでターミナルはUNIXのshellの中でzshというshellを採用していますが、

ターミナル（zsh）を起動するとき

上の内容については他のポストで説明します、今は.zshrcとzprofileの違いを知るだけで十分です。
